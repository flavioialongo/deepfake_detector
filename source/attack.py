...
class AdversarialAttacker:
    ...
    def __init__(self, model, 
                 ...
                 iterative_alpha=None,
                 iterative_epsilon=None,
                 deepfool_overshoot = None,
                 deepfool_maxiter = None):
        ...
        if attack_type == "fgsm":
            ...
            if(iterative_steps == None):
                raise Exception("PGD chosen but no steps specified")
            if(iterative_alpha == None):
                 raise Exception("PGD chosen but no alpha specified")
            if(iterative_epsilon == None):
                 raise Exception("PGD chosen but no epsilon specified")
        
            self.pgd_steps = iterative_steps
            self.pgd_alpha = iterative_alpha
            self.pgd_epsilon = iterative_epsilon

        elif attack_type == "ifgsm":
            self.attack = self.i_fgsm_attack

            if(iterative_steps == None):
                raise Exception("I-FGSM chosen but no steps specified")
            if(iterative_alpha == None):
                 raise Exception("I-FGSM chosen but no alpha specified")
            if(iterative_epsilon == None):
                 raise Exception("I-FGSM chosen but no epsilon specified")

            self.ifgsm_steps = iterative_steps
            self.ifgsm_alpha = iterative_alpha
            self.ifgsm_epsilon = iterative_epsilon
        
        elif attack_type == "deepfool":
            if(deepfool_overshoot == None):
                raise Exception("Deepfool chosen but no overshoot specified")
            if(deepfool_maxiter == None):
                 raise Exception("Deepfool chosen but no max_iter specified")
            
            self.deepfool_overshoot = deepfool_overshoot
            self.deepfool_maxiter = deepfool_maxiter
    ...
    def i_fgsm_attack(self, images, labels, epsilon, alpha, iters):
        ...
        for _ in range(iters):
            ...
            x_adv = torch.clamp(x_adv, 0, 1).detach()  # keep in valid image range
    ...
    def pgd_attack(self, images, labels, epsilon, alpha, steps):
        mean = self.mean
        std = self.std
        images_denorm = images * std + mean  # unnormalize
        model = self.model

        # 🔹 Random initialization within epsilon ball
        delta = torch.empty_like(images_denorm).uniform_(-epsilon, epsilon)
        adv_images = torch.clamp(images_denorm + delta, 0, 1).detach()
    ...
    def deepfool_attack(self, image, overshoot=0.02, max_iter=50):
    ...
    def evaluate_attack(self, dataloader, save_path=None, save_name="adv_analysis", epsilon=0.01, visualize=False, num_visualize=5):
        ...
        for batch in tqdm(dataloader, desc="Evaluating adversarial robustness"):
            ...
            if len(correctly_classified_indices) > 0:
                ...
                correct_outputs = self.model(correct_images)
                loss = F.cross_entropy(correct_outputs, correct_labels)
                loss.backward()
                
                image_grad = correct_images.grad.data
                

                # Choose correct attack type 
                if self.attack_type == "fgsm":
                    adv_images = self.attack(correct_images, epsilon, image_grad)
                elif self.attack_type == "pgd":
                    adv_images = self.attack(correct_images, correct_labels, self.pgd_epsilon, 
                                        self.pgd_alpha, self.pgd_steps)
                elif self.attack_type == "ifgsm":
                    adv_images = self.attack(correct_images, correct_labels, self.ifgsm_epsilon, self.ifgsm_alpha, self.ifgsm_steps)
                else:
                    adv_images = self.attack(correct_images, self.deepfool_overshoot, self.deepfool_maxiter)
                ...
                if visualize and visualization_count < num_visualize and len(correctly_classified_indices) > 0:
                    self._visualize_adversarial_examples(
                        ...
                        epsilon, min(num_visualize - visualization_count, len(correctly_classified_indices))
                    )
            ...
            self.model.zero_grad()
            outputs_for_grad = self.model(images_for_class_analysis)
            loss_for_grad = F.cross_entropy(outputs_for_grad, labels)
            loss_for_grad.backward()
            
            if self.attack_type == "fgsm":
                all_adv_images = self.attack(images_for_class_analysis, epsilon, images_for_class_analysis.grad.data)
            elif self.attack_type == "pgd":
                all_adv_images = self.attack(images_for_class_analysis, labels, self.pgd_epsilon, 
                                    self.pgd_alpha, self.pgd_steps)
            elif self.attack_type == "ifgsm":
                all_adv_images = self.attack(images_for_class_analysis, labels, self.ifgsm_epsilon, self.ifgsm_alpha, self.ifgsm_steps)
            else:
                all_adv_images = self.attack(images_for_class_analysis, self.deepfool_overshoot, self.deepfool_maxiter)
        ...
        # === REPORTING ===
        with open(os.path.join(save_path, "analysis.txt"), 'a') as f:
            ...
            # === REPORTING ===
            write_and_print(f"\n{'='*60}")
            write_and_print(f"ADVERSARIAL EVALUATION RESULTS (ε = {epsilon})")
            
            attack_param_str = f"Attack: {self.attack_type}"
            if self.attack_type.lower() == 'fgsm':
                pass  
            elif self.attack_type.lower() == 'pgd':
                attack_param_str += f" | pgd epsilon: {self.pgd_epsilon} | pgd alpha: {self.pgd_alpha} | pgd steps: {self.pgd_steps}"
            elif self.attack_type.lower() == 'ifgsm':
                attack_param_str += f" | ifgsm epsilon: {self.ifgsm_epsilon} | ifgsm alpha: {self.ifgsm_alpha} | ifgsm steps: {self.ifgsm_steps}"
            else:
    ...
    def _save_analysis_plots(self, clean_labels, clean_preds, adv_labels, adv_preds,
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_path, f"{save_name}_confusion_matrices.png"), dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if len(real_clean_preds) > 0 and len(fake_clean_preds) > 0:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(save_path, f"{save_name}_class_specific_analysis.png"), dpi=300, bbox_inches='tight')
            plt.close()
